...
public class Act4_Server implements Act4_Server_Interface
{
	...
	public void anunciarPresencia(String nomClient, int heartbeat) throws RemoteException
	{
		...
		System.out.println("Nou client afegit a la taula; es diu "+nomClient+" i te un heartbeat de "+heartbeat+"s");
	}
	...
	{
		for(int i=0; i<taula.size(); i++) if(taula.get(i).nom.toString().equals(nomClient))
		{
	...
	public static void main(String[] args)
	{
		...
		for(;;)
		{
			...
			for(;;) if(System.nanoTime()>=targetTime)
			{
				actualitzarTaula();
				break;
			}
	...
	{
		for(int i=0; i<taula.size(); i++) if(taula.get(i).estat)
		{
			...
			if(taula.get(i).passadesServidor>=(taula.get(i).heartbeat * 2))
			{
				...
				System.out.println("Fa "+(taula.get(i).heartbeat * 2)+"s que no rebo res desde "+taula.get(i).nom+". El marco com inactiu.");
			}
		}
	}
	
//	public synchronized void escriureTaula()
//	{
//
//	}
}